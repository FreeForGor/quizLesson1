package ru.umsch.quiz.quiz.cli;
...
import ru.umsch.quiz.quiz.service.MessageService;
...
public class TesterCLI {
    ...
    @ShellMethod(value = "Define language: english (\"en\") or russian (\"ru\")", key = "lang")
    public void defineLanguage(
        ...
        switch (lang) {
            ...
            case "ru":
                ...
                break;
            default:
                messenger.textMessage("There is only two options: english (\"en\") or russian (\"ru\")");
        }
    }

    @ShellMethod(value = "Show all testing results", key = "show-all")
    public String showAllResults() {
        List<TestRes> testResults = tester.getTestResults();
        ...
        for (TestRes testResult : testResults) {
    ...
    public String showResultsForName(
        ...
        List<TestRes> testResults = tester.getTestResults();
        Optional<TestRes> optional = testResults.stream()
        ...
        TableModelBuilder<String> modelBuilder = new TableModelBuilder<>();
        TestRes testResult = optional.get();
        addResultsToModelBuilder(modelBuilder, testResult);
    ...
    private void  addResultsToModelBuilder(TableModelBuilder<String> modelBuilder, TestRes testResult) {
        ...
        testResult.getResults().forEach((question, grade) -> modelBuilder.addRow()
                .addValue(question)
                .addValue(grade));
        modelBuilder.addRow()
